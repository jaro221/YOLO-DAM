...
class ConvBNAct(L.Layer):
    ...
    def __init__(self, filters, k=1, s=1, g=1, act=True, name=None):
        ...
        self.bn = L.BatchNormalization(name=None if name is None else name + "_bn")
        self.act = L.Activation(SiLU, name=None if name is None else name + "_silu") if act else None
...
class Bottleneck(L.Layer):
    ...
    def __init__(self, c, shortcut=True, e=0.5, name=None):
        ...
        self.cv2 = ConvBNAct(c, 3, 1, name=None if name is None else name + "_cv2")
